...
function RunHistoryRow({ run }: { run: MonitorRun }) {
  ...
  return (
    <div style={{
      ...
      <span style={{ color: "#6b6860", fontFamily: "ui-monospace, 'Cascadia Code', Menlo, monospace", fontSize: 11, flexShrink: 0 }}>
        {new Date(run.ran_at).toLocaleString()}
      </span>
      {run.actual_value !== null && (
        <span style={{ color: "#9060f8", fontWeight: 500, flexShrink: 0 }}>
          {typeof run.actual_value === "number" ? run.actual_value.toFixed(2) : run.actual_value}
        </span>
      )}
      {run.message && (
        <span style={{ color: "#6b6860", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
          {run.message}
        </span>
      )}
    </div>
      ...
      <div style={{ padding: "14px 16px" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          <button
            ...
            style={{ background: "none", border: "none", cursor: "pointer", padding: 0, color: "#9a9690", flexShrink: 0 }}
          >
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
              <span style={{ fontWeight: 600, fontSize: 14, color: "#111010", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                {monitor.name}
              </span>
              <span style={{
                fontSize: 10, fontWeight: 500, padding: "1px 6px", borderRadius: 20,
                background: monitor.is_active ? "rgba(144,96,248,0.1)" : "rgba(0,0,0,0.06)",
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 4, flexShrink: 0 }}>
            ...
            <button
              ...
              title="Run now"
              style={{
                background: "rgba(144,96,248,0.1)", border: "none", cursor: "pointer",
                padding: "4px 8px", borderRadius: 6, color: "#9060f8",
        ...
        {expanded && (
          <div style={{ marginTop: 12, paddingTop: 10, borderTop: "1px solid rgba(0,0,0,0.06)" }}>
            <p style={{
              ...
              color: "#9a9690", marginBottom: 6,
              fontFamily: "ui-monospace, 'Cascadia Code', Menlo, monospace",
            }}>
              Run history
            </p>
            {runsLoading ? (
              <div style={{ display: "flex", alignItems: "center", gap: 6, color: "#9a9690", fontSize: 12 }}>
                <Loader2 size={12} style={{ animation: "spin 1s linear infinite" }} /> Loading…
              </div>
            ) : runs.length === 0 ? (
              <p style={{ fontSize: 12, color: "#9a9690" }}>No runs yet. Click Run now to trigger the first check.</p>
            ) : (
              <div>
                {runs.map((r) => <RunHistoryRow key={r.run_id} run={r} />)}
              </div>
            )}
          </div>
        )}
  ...
  return (
    <div style={{ padding: "24px 32px", maxWidth: 820, margin: "0 auto" }}>
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 24 }}>
        <div>
          <h2 style={{ fontSize: 20, fontWeight: 700, color: "#111010", margin: 0 }}>Monitors</h2>
      ...
      {loading ? (
        ...
        <div style={{
          ...
          <div style={{ textAlign: "center" }}>
            <p style={{ fontWeight: 600, fontSize: 15, color: "#111010", margin: "0 0 4px" }}>No monitors yet</p>
            <p style={{ fontSize: 13, color: "#9a9690", margin: 0 }}>
              Create a monitor to track row count, null rates, quality score, or column drift.
            </p>
          </div>
          <button
            onClick={() => setModalOpen(true)}
            style={{
              ...
              background: "rgba(144,96,248,0.1)", color: "#9060f8",
              fontSize: 13, fontWeight: 600, border: "1px solid rgba(144,96,248,0.2)",
        ...
        <>
          ...
          <div style={{ display: "flex", gap: 16, marginBottom: 20 }}>
            {[
              ...
              { label: "Active",  value: monitors.filter((m) => m.is_active).length,                        color: "#9060f8" },
              { label: "Firing",  value: monitors.filter((m) => m.last_status === "firing").length,          color: "#ef4444" },
              { label: "OK",      value: monitors.filter((m) => m.last_status === "ok").length,              color: "#22c55e" },
