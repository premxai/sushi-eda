...
function saveHistory(h: HistoryEntry[]) {
  try {
    localStorage.setItem(
      STORAGE_HISTORY_KEY,
      JSON.stringify(h.slice(0, MAX_HISTORY)),
    );
  } catch {
    /* quota */
  }
...
function saveSaved(s: SavedQuery[]) {
  try {
    localStorage.setItem(STORAGE_SAVED_KEY, JSON.stringify(s));
  } catch {
    /* quota */
  }
}
...
export function SQLQuerySection({ datasetId, orgId = "default" }: Props) {
  ...
  const [sidePanel, setSidePanel] = useState<"schema" | "history" | "saved">(
    "schema",
  );
  ...
  const handleRun = useCallback(
    async (nextOffset?: number) => {
      ...
      try {
        ...
        // Prepend to history (functional update to avoid depending on history)
        ...
        setHistory((prev) => {
          ...
          const newHist = [entry, ...prev.filter((h) => h.sql !== trimmedSql)];
          saveHistory(newHist);
          return newHist;
        });
      } catch (e: unknown) {
    ...
    [datasetId, sql, limit, orgId, offset],
  );
  ...
  return (
    ...
    >
      {needsBackend && (
        ...
        >
          ⚠️ SQL queries require a live backend connection. Upload your file
          with the backend running to enable the SQL Editor.
        </div>
      )}
            ...
            >
              ...
              >
                ...
                >
                  <button
                    onClick={() =>
                      void handleRun(Math.max(0, result.offset - result.limit))
                    }
                      ...
                      <div
                        ...
                        onClick={() =>
                          setSql((q) =>
                            q.includes(`"${col.name}"`)
                              ? q
                              : q.replace(/SELECT \*/, `SELECT "${col.name}"`),
                          )
                        }
